...
class BaseChangeList(ChangeList):
    def url_for_result(self, result):
        ...
        #     args=(quote(pk),),
        #     current_app=self.model_admin.admin_site.name,
        # ) #change para alterar o objeto

class BaseModelAdmin(ImportExportMixin, ExportActionMixin, SafeDeleteAdmin):
    ...
    def get_urls(self):
        ...
        urls.append(path('view/<path:object_id>/', wrap(self.preview_view), name=f'{prefix}_view'))
        return urls
    ...
    def preview_view(self, request, object_id, form_url="", extra_context=None):
        ...
        context = {
            **self.admin_site.each_context(request),
            "title": ("View %s") % self.opts.verbose_name,
