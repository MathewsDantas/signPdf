...
class DocumentAdmin(BaseModelAdmin):
    ...
    list_display = ('client', 'document_title', 'document_body', 'document_signature', 'document_hash')
    search_fields = ('client', 'document_title', 'document_body', 'document_signature', 'document_hash')
    fieldsets = [
        (("Identificação"), {"fields": [ "client"]}),
        (
            ("Mais infos"),
            {"fields": ["document_title", "document_body", "document_signature", "document_hash"]},
        ),
    ]
